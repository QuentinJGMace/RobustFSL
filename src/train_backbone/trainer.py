...
class Trainer:
    def __init__(self, device, preprocess, args):
        self.device = device
        self.args = args
    ...
    def do_epoch(
        ...
        for i, (data, target) in enumerate(tqdm_train_loader):
            ...
            if i % print_freq == 0:
                ...
                self.log_to_wandb(losses.avg, top1.avg)
        scheduler.step()
    ...
    def smooth_one_hot(self, targets):
        ...
        with torch.no_grad():
            new_targets = torch.empty(
                size=(targets.size(0), self.num_classes_train), device=self.device
            )
            new_targets.fill_(self.args.label_smoothing / (self.num_classes_train - 1))
            new_targets.scatter_(
                1, targets.unsqueeze(1), 1.0 - self.args.label_smoothing
            )
        return new_targets
        ...
        with torch.no_grad():
            ...
            for i, (datas, target) in enumerate(tqdm_test_loader):
                ...
                train_out = train_out.reshape(
                    self.args.meta_val_way, self.args.meta_val_shot, -1
                ).mean(1)
